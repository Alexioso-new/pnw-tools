/* CastFlow Dual Free Canvas — v117 / v9.17 */
(function(){
'use strict';
var VERSION='v9.17', KEY='pnwCastflowDualCanvas.v1', MIN=.12, MAX=3, HEAD=30;
...
if(/^(display|stage|remote|youthviews)$/i.test(qmode)) return;
var stage,root,view,world,menu,help,zoomLabel,mini,empty,editMark,liveMark,state;
var active=false,selected='',spaceDown=false,op=null,saveTimer=0,miniRaf=0,miniTimer=0,miniLast=0,paintRaf=0,viewDirty=false,nodeDirty={},visRaf=0,fresh=false;
function queuePaint(){if(paintRaf)return;paintRaf=requestAnimationFrame(function(){paintRaf=0;flushPaint();});}
function flushPaint(){if(paintRaf){cancelAnimationFrame(paintRaf);paintRaf=0;}if(viewDirty){viewDirty=false;paintViewNow();}var ids=Object.keys(nodeDirty);nodeDirty={};ids.forEach(function(x){var n=find(x);if(n)paintNodeNow(n);});}
...
function iframe(src,title){var f=document.createElement('iframe');f.className='cfDcIframe';f.title=title;f.src=src;f.loading='lazy';f.setAttribute('allow','autoplay; fullscreen; clipboard-read; clipboard-write');f.addEventListener('load',function(){var b=f.closest?f.closest('.cfDcFrame'):null;postVisible(f,!!(active&&b&&!b.classList.contains('is-offscreen')));});return f;}
function paintNodeNow(n){if(!world)return;var b=world.querySelector('[data-dc-id=\"'+n.id+'\"]');if(!b)return;b.style.left=n.x+'px';b.style.top=n.y+'px';b.style.width=n.w+'px';b.style.height=n.h+'px';b.style.zIndex=n.z;b.classList.toggle('is-selected',selected===n.id);b.classList.toggle('is-locked',n.locked);var r=b.querySelector('[data-act=\"ratio\"]');if(r)r.classList.toggle('on',n.ratio);var l=b.querySelector('[data-act=\"lock\"]');if(l)l.textContent=n.locked?'🔒':'♢';scheduleMini();scheduleVisibility();}
function styleNode(n,now){if(!n)return;nodeDirty[n.id]=true;if(now)flushPaint();else queuePaint();}
function render(){
...
function paintViewNow(){if(!world)return;world.style.transform='translate3d('+state.view.x+'px,'+state.view.y+'px,0) scale('+state.view.zoom+')';zoomLabel.textContent=Math.round(state.view.zoom*100)+'%';root.classList.toggle('no-grid',!state.grid);view.style.setProperty('--dc-small',Math.max(6,20*state.view.zoom)+'px');view.style.setProperty('--dc-big',Math.max(30,100*state.view.zoom)+'px');view.style.setProperty('--dc-x',state.view.x+'px');view.style.setProperty('--dc-y',state.view.y+'px');scheduleMini();scheduleVisibility();}
function applyView(now){viewDirty=true;if(now)flushPaint();else queuePaint();}
function scheduleMini(){if(miniRaf||miniTimer)return;var wait=Math.max(0,66-(performance.now()-miniLast));if(wait>1){miniTimer=setTimeout(function(){miniTimer=0;scheduleMini();},wait);return;}miniRaf=requestAnimationFrame(function(){miniRaf=0;miniLast=performance.now();renderMini();});}
...
function pointerEnd(e){if(!op||(e.pointerId!=null&&e.pointerId!==op.pid))return;flushPaint();updateVisibility();op=null;root.classList.remove('is-panning','is-dragging','is-resizing');save();}
...
function postVisible(f,on){if(!f||!f.contentWindow)return;try{f.contentWindow.postMessage({type:'cf:previewVisibility',visible:!!on},'*');}catch(e){}}
function updateVisibility(){if(!active||!view||!world)return;var vr=view.getBoundingClientRect(),z=state.view.zoom||1,vx=-state.view.x/z,vy=-state.view.y/z,vw=vr.width/z,vh=vr.height/z,pad=120/z;state.nodes.forEach(function(n){var on=n.x+n.w>=vx-pad&&n.y+n.h>=vy-pad&&n.x<=vx+vw+pad&&n.y<=vy+vh+pad,b=world.querySelector('[data-dc-id="'+n.id+'"]');if(!b)return;b.classList.toggle('is-offscreen',!on);if(b._cfVisible===on)return;b._cfVisible=on;Array.prototype.forEach.call(b.querySelectorAll('iframe'),function(f){postVisible(f,on);});try{document.dispatchEvent(new CustomEvent('cf:dualFrameVisibility',{detail:{id:n.id,type:n.type,visible:on}}));}catch(e){}});}
function scheduleVisibility(){if(visRaf||!active)return;visRaf=requestAnimationFrame(function(){visRaf=0;updateVisibility();});}
function syncModeVisibility(){if(!stage)return;var pm=stage.dataset.previewMode||'edit',live=id('cfLiveFrame');postVisible(live,!active&&pm==='live');if(root&&!active)Array.prototype.forEach.call(root.querySelectorAll('iframe'),function(f){if(f!==live)postVisible(f,false);});try{document.dispatchEvent(new CustomEvent('cf:dualFrameVisibility',{detail:{id:'edit-native',type:'edit',visible:!active&&pm==='edit'}}));}catch(e){}}
function activate(){if(!root||active)return;active=true;root.hidden=false;root.classList.add('is-active');stage.classList.add('cfDcMode');render();applyView();if(fresh){fresh=false;requestAnimationFrame(function(){fit(true);});}}
function deactivate(){if(!root)return;var was=active;active=false;root.classList.remove('is-active');spaceDown=false;op=null;root.hidden=true;stage.classList.remove('cfDcMode','cfDcMax');if(was)restore();menu.hidden=true;help.hidden=true;syncModeVisibility();}
function setActive(on){if(on)activate();else deactivate();}
function build(){stage=id('cfPrevStage');var edit=id('projPreview'),live=id('cfLiveFrame');if(!stage||!edit||!live||id('cfDualCanvas'))return false;editMark=marker(edit,'edit');liveMark=marker(live,'live');live.addEventListener('load',function(){if(active)updateVisibility();else syncModeVisibility();});state=load();root=document.createElement('div');root.id='cfDualCanvas';root.className='cfDualCanvas';root.hidden=true;root.innerHTML='<div class="cfDcToolbar"><button class="primary" data-tool="add">＋ Output</button><i></i><button data-tool="zout">−</button><button class="zoom" data-tool="actual">100%</button><button data-tool="zin">＋</button><button data-tool="fit">Fit</button><i></i><button class="on" data-tool="grid">Grid</button><button data-tool="reset">Reset</button><button data-tool="max">⛶</button><button data-tool="help">?</button><span>Drag kosong = pan · Ctrl/⌘ + wheel = zoom</span></div><div class="cfDcViewport" tabindex="0"><div class="cfDcWorld"></div><div class="cfDcEmpty" hidden><b>Canvas kosong</b><small>Gunakan + Output untuk menambahkan layar.</small></div><div class="cfDcMini" title="Klik untuk Fit"></div></div><div class="cfDcMenu" hidden><b>Tambah output</b><button data-add="edit">✎ Edit Preview</button><button data-add="live">● Live Output</button><button data-add="stage">▣ Stage Display</button><button data-add="remote">⌁ Remote Control</button><label>Browser / URL</label><div><input id="cfDcUrl" placeholder="https://…"><button id="cfDcUrlAdd">Tambah</button></div></div><div class="cfDcHelp" hidden><b>Dual Free Canvas</b><span><kbd>Space</kbd> + drag / area kosong — Pan</span><span><kbd>Ctrl/⌘</kbd> + wheel — Zoom</span><span><kbd>Arrow</kbd> 1 px · <kbd>Shift</kbd> 10 px</span><span><kbd>Ctrl/⌘ D</kbd> Duplikat · <kbd>Delete</kbd> Keluarkan</span><span><kbd>0</kbd> Fit · <kbd>1</kbd> 100% · <kbd>Esc</kbd> keluar</span><small>Alt saat resize melepas rasio sementara.</small></div>';
