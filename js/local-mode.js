...
(function () {
  ...
  function log() {
    try {
      console.log.apply(console, ["[Mode Lokal]"].concat([].slice.call(arguments)));
    } catch (e) {}
  }
  ...
  function seedFromDevice() {
    try {
      ...
      if (isEmpty(songs) && localSongs && localSongs.length) {
        log("server masih kosong - mengunggah " + localSongs.length + " lagu dari perangkat ini");
        setPathRemote("pujianYouth/songs", localSongs);
      }
      if (isEmpty(bank) && localBank && localBank.length) {
        log("mengunggah " + localBank.length + " lagu Song Bank dari perangkat ini");
    ...
    var ref = {
      ...
      transaction: function (fn) {
        ...
        if (typeof cur === "number" || typeof next === "number") {
          ...
          fireFor(full);
          return api("/api/incr", { path: full, delta: delta }).then(function () {
    ...
    var authObj = {
      ...
      createUserWithEmailAndPassword: function () {
        return Promise.reject(
          new Error("Mode Lokal: pendaftaran akun butuh internet. Pakai login admin lokal."),
        );
      },
      signInWithEmailAndPassword: function () {
        return Promise.reject(
          new Error("Mode Lokal: login email butuh internet. Pakai login admin lokal."),
        );
      },
      signInWithPopup: function () {
        return Promise.reject(
          new Error("Mode Lokal: login Google butuh internet. Pakai login admin lokal."),
        );
      },
    };
    return authObj;
