/* PNW-FILE-GUIDE
   js/logger.js — window.PNWLog, pencatat debug.
   Dipakai js/app.js & js/recorder.js. Dimuat paling awal (defer) di index.html.
 */
...
(function () {
  ...
  function base() {
    return {
      ...
      online:
        navigator && typeof navigator.onLine === "boolean"
          ? navigator.onLine
          : null,
      ...
      enqueue({
        ...
        stack:
          e && e.error && e.error.stack
            ? String(e.error.stack).slice(0, 1500)
            : "",
      });
    },
    true,
    ...
    ready: function (firebaseNs, context) {
      ...
      if (context)
        for (var k in context)
          if (context.hasOwnProperty(k)) ctx[k] = context[k];
      ready = !!fb;
      flush();
    },
